...
\section{Statistik}
\subsection{Basics}
Mittelwert $\bar{x}=\frac{1}{n}\sum^n_{i=1}x_i$\\
Varianz $\sigma^2 = \frac{1}{n-1} \sum^n_{i=1}(x_i-\bar{x})^2$\\
\vspace{6pt}
Kovarianz \\
...
Jahresmittel Geschw. $v_m = A \cdot \Gamma(1+\frac{1}{k}) \approx A \cdot (0.568+\frac{0.434}{k})^\frac{1}{k}$  \\
\vspace{7pt}
Gammafunktion $\Gamma(x) = \int_0^{\infty}\text{exp}(-t)t^{x-1} dt$\\
Quadratisches Mittel $\bar{u}^2 =  A^2 \cdot \Gamma(1+\frac{2}{k})$\\
Kubisches Mittel $\bar{u}^3 =  A^3 \cdot \Gamma(1+\frac{3}{k})$\\
...
Rotorleistung $P_{\text{Rotor}} = \frac{P_{\text{elektrisch}}}{\eta_e} = P_{\text{Wind}} \cdot c_P$\\
...
\section{Skalierungsgr\"o{\ss}en}


\begin{tabular}{lc}
Kenngr\"o{\ss}e                           & Exponent des Durchmessers \\ \hline 
Belastungen:                        &                           \\
- Drehzahl                          & -1                        \\
- Leistung, Luftkr\"afte            & 2                         \\
- Drehmoment, Massen                & 3                         \\
Beanspruchung durch:                &                           \\
- Massen                            & 1                         \\
- Luftkr\"afte, Fliehkr\"afte           & 0                         \\
\end{tabular}
...
\subsection{Systeme von Feder-D\"ampfer Schwingern}
Bewegungsgleichungssystem $\textbf{M}\ddot{q} + \textbf{B} \dot{q} + \textbf{K} = \textbf{f}$\\
\vspace{3pt}
Eigenwerte bestimmen durch $\text{det}(\lambda^2\textbf{M}-\textbf{K})$\\
\vspace{3pt}
Schwingungsmoden aus Eigenvektoren von $\lambda^2\textbf{M}-\textbf{K}$
...
\vspace{7pt}
Zwei Trafo Typen - Trocken und mit \"Ol gefüllt (synthetisches \"Ol, Silikon\"ol, Ester); Unterschiede in Wartungsaufwand, Flammbarkeit, K\"uhleigenschaften, Isolationseigenschaften, Kompaktbauweise und Kosten.\\
\vspace{7pt}
...
\begin{itemize}
	\item Direkte Koppelung des Stators des Asynchrongenerators an das Netz $\rightarrow$ im Stator Netzfrequenz
